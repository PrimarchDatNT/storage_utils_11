...
package office.java.awt.geom;

import java.io.Serializable;
import java.io.StreamCorruptedException;

import office.java.awt.Rectangle;
import office.java.awt.Shape;
...
public abstract class Path2D implements Shape, Cloneable
{
    ...
    public static class Float extends Path2D implements Serializable
    {
            ...
            for (int i = 1; i < numTypes; i++)
            {
                switch (pointTypes[i])
                {
                    ...
                    case PathIterator.SEG_QUADTO:
                        crossings += Curve.pointCrossingsForQuad(px, py, curx, cury, coords[ci++],
                            coords[ci++], endx = coords[ci++], endy = coords[ci++], 0);
                        curx = endx;
                        cury = endy;
                        break;
                    case PathIterator.SEG_CUBICTO:
                        crossings += Curve.pointCrossingsForCubic(px, py, curx, cury, coords[ci++],
                            coords[ci++], coords[ci++], coords[ci++], endx = coords[ci++],
                            endy = coords[ci++], 0);
                        curx = endx;
                        cury = endy;
                        break;
        ...
        int rectCrossings(double rxmin, double rymin, double rxmax, double rymax)
        {
            ...
            for (int i = 1; crossings != Curve.RECT_INTERSECTS && i < numTypes; i++)
            {
                switch (pointTypes[i])
                {
                    case PathIterator.SEG_MOVETO:
                        ...
                        {
                            crossings = Curve.rectCrossingsForLine(crossings, rxmin, rymin, rxmax,
                    ...
                    case PathIterator.SEG_LINETO:
                        crossings = Curve.rectCrossingsForLine(crossings, rxmin, rymin, rxmax,
                            rymax, curx, cury, endx = coords[ci++], endy = coords[ci++]);
                        curx = endx;
                        cury = endy;
                        break;
                    case PathIterator.SEG_QUADTO:
                        crossings = Curve.rectCrossingsForQuad(crossings, rxmin, rymin, rxmax,
                            rymax, curx, cury, coords[ci++], coords[ci++], endx = coords[ci++],
                            endy = coords[ci++], 0);
                        curx = endx;
                        cury = endy;
                        break;
                    case PathIterator.SEG_CUBICTO:
                        crossings = Curve.rectCrossingsForCubic(crossings, rxmin, rymin, rxmax,
                            rymax, curx, cury, coords[ci++], coords[ci++], coords[ci++],
                            coords[ci++], endx = coords[ci++], endy = coords[ci++], 0);
                        curx = endx;
                        cury = endy;
                        break;
            ...
            {
                crossings = Curve.rectCrossingsForLine(crossings, rxmin, rymin, rxmax, rymax, curx,
                    cury, movx, movy);
            }*/
            // Count should always be a multiple of 2 here.
            // assert((crossings & 1) != 0);
    ...
    public static boolean contains(PathIterator pi, double x, double y)
    {
        if (x * 0.0 + y * 0.0 == 0.0)
        {
            ...
            int cross = 0;//Curve.pointCrossingsForPath(pi, x, y);
            return ((cross & mask) != 0);
        }
        else
        {
    ...
    public static boolean contains(PathIterator pi, double x, double y, double w, double h)
    {
        ...
        int crossings = 0;//Curve.rectCrossingsForPath(pi, x, y, x + w, y + h);
        return (crossings != 0/*Curve.RECT_INTERSECTS*/ && (crossings & mask) != 0);
    }
