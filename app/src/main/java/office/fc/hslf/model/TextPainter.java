...
public final class TextPainter
{
    ...
    /*public void paint(Graphics2D graphics)
    {
        ...
        switch (valign)
        {
            ...
            case TextShape.AnchorMiddle:
                float delta = (float)anchor.getHeight() - textHeight - _shape.getMarginTop()
                    - _shape.getMarginBottom();
        ...
        for (int i = 0; i < elem.length; i++)
        {
            ...
            switch (elem[i]._align)
            {
                ...
                case TextShape.AlignCenter:
                    pen.x = anchor.getX()
                        + _shape.getMarginLeft()
                        + (anchor.getWidth() - elem[i].advance - _shape.getMarginLeft() - _shape
                            .getMarginRight()) / 2;
                    break;
            ...
            {
                graphics.drawString(elem[i]._bullet.getIterator(),
                    (float)(pen.x + elem[i]._bulletOffset), (float)pen.y);
            }
    ...
    /*public TextElement[] getTextElements(float textWidth, FontRenderContext frc)
    {
        ...
        while (measurer.getPosition() < paragraphEnd)
        {
            ...
            RichTextRun rt = run.getRichTextRunAt(startIndex == text.length() ? (startIndex - 1)
                : startIndex);
            if (rt == null)
            {
                logger.log(POILogger.WARN, "RichTextRun not found at pos" + startIndex
            ...
            if (ruler != null)
            {
                int bullet_val = ruler.getBulletOffsets()[indent] * Shape.POINT_DPI
                    / Shape.MASTER_DPI;
            ...
            TextLayout textLayout = measurer.nextLayout(wrappingWidth + 1, nextBreak == -1
                ? paragraphEnd : nextBreak, true);
            if (textLayout == null)
            {
                textLayout = measurer.nextLayout(textWidth, nextBreak == -1 ? paragraphEnd
                    : nextBreak, false);
            }
            if (textLayout == null)
            {
                logger.log(POILogger.WARN, "Failed to break text into lines: wrappingWidth: "
                    + wrappingWidth + "; text: " + rt.getText());
            ...
            if (rt.isBullet() && (prStart || startIndex == 0))
            {
                ...
                if (clr != null)
                    ...
                    bat.addAttribute(TextAttribute.FOREGROUND,
                        it.getAttribute(TextAttribute.FOREGROUND));
                ...
                if (!new Font(bulletFont.getFontName(), Font.PLAIN, 1).canDisplay(rt
                    .getBulletChar()))
                {
                    bat.addAttribute(TextAttribute.FAMILY, "Arial");
                    bat = new AttributedString("" + DEFAULT_BULLET_CHAR, bat.getIterator()
                        .getAttributes());
                }
