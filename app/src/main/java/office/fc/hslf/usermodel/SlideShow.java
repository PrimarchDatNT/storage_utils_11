...
package office.fc.hslf.usermodel;
...
import office.fc.hslf.exceptions.CorruptPowerPointFileException;
import office.fc.hslf.model.HeadersFooters;
import office.fc.hslf.model.Hyperlink;
import office.fc.hslf.model.MovieShape;
...
import office.fc.hslf.record.Document;
import office.fc.hslf.record.DocumentAtom;
import office.fc.hslf.record.ExAviMovie;
import office.fc.hslf.record.ExControl;
import office.fc.hslf.record.ExHyperlink;
import office.fc.hslf.record.ExHyperlinkAtom;
import office.fc.hslf.record.ExMCIMovie;
import office.fc.hslf.record.ExObjList;
import office.fc.hslf.record.ExObjListAtom;
import office.fc.hslf.record.ExOleObjAtom;
import office.fc.hslf.record.ExVideoContainer;
import office.fc.hslf.record.ExtendedParagraphHeaderAtom;
import office.fc.hslf.record.ExtendedPresRuleContainer;
import office.fc.hslf.record.FontCollection;
import office.fc.hslf.record.HeadersFootersContainer;
import office.fc.hslf.record.PersistPtrHolder;
import office.fc.hslf.record.PositionDependentRecord;
import office.fc.hslf.record.PositionDependentRecordContainer;
import office.fc.hslf.record.Record;
import office.fc.hslf.record.RecordContainer;
import office.fc.hslf.record.RecordTypes;
import office.fc.hslf.record.SlideListWithText;
import office.fc.hslf.record.SlidePersistAtom;
import office.fc.hslf.record.ExtendedPresRuleContainer.ExtendedParaAtomsSet;
import office.fc.hslf.record.SlideListWithText.SlideAtomsSet;
    ...
    private void buildSlidesAndNotes()
    {
        ...
        if (masterSLWT != null)
        {
            ...
            for (int i = 0; i < masterSets.length; i++)
            {
                ...
                if (r instanceof office.fc.hslf.record.Slide)
                {
                    TitleMaster master = new TitleMaster((office.fc.hslf.record.Slide)r, sheetNo);
                    master.setSlideShow(this);
                    tmr.add(master);
                }
                else if (r instanceof office.fc.hslf.record.MainMaster)
                {
                    SlideMaster master = new SlideMaster((office.fc.hslf.record.MainMaster)r, sheetNo);
                    master.setSlideShow(this);
                    mmr.add(master);
                }
            }
        ...
        // notesSLWT
        office.fc.hslf.record.Notes[] notesRecords;
        SlideAtomsSet[] notesSets = new SlideAtomsSet[0];
        Hashtable<Integer, Integer> slideIdToNotes = new Hashtable<Integer, Integer>();
        if (notesSLWT == null)
        {
            // None
            notesRecords = new office.fc.hslf.record.Notes[0];
        }
        else
        {
            // Match up the records and the SlideAtomSets
            notesSets = notesSLWT.getSlideAtomsSets();
            ArrayList<office.fc.hslf.record.Notes> notesRecordsL = new ArrayList<office.fc.hslf.record.Notes>();
            for (int i = 0; i < notesSets.length; i++)
            {
                ...
                if (r instanceof office.fc.hslf.record.Notes)
                {
                    office.fc.hslf.record.Notes notesRecord = (office.fc.hslf.record.Notes)r;
            ...
            notesRecords = new office.fc.hslf.record.Notes[notesRecordsL.size()];
            notesRecords = notesRecordsL.toArray(notesRecords);
        }

        // Now, do the same thing for our slides
        office.fc.hslf.record.Slide[] slidesRecords;
        SlideAtomsSet[] slidesSets = new SlideAtomsSet[0];
        if (slidesSLWT == null)
        {
            // None
            slidesRecords = new office.fc.hslf.record.Slide[0];
        }
        else
        {
            // Match up the records and the SlideAtomSets
            slidesSets = slidesSLWT.getSlideAtomsSets();
            slidesRecords = new office.fc.hslf.record.Slide[slidesSets.length];
            for (int i = 0; i < slidesSets.length; i++)
            {
                ...
                if (r instanceof office.fc.hslf.record.Slide)
                {
                    slidesRecords[i] = (office.fc.hslf.record.Slide)r;
                }
                /*else
                {
