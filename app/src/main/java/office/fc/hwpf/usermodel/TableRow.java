...
import office.fc.hwpf.sprm.TableSprmUncompressor;
    ...
    private void initCells()
    {
        ...
        for (int p = 0; p < numParagraphs(); p++)
        {
                ...
                .isEmbeddedCellMark()) && paragraph.getTableLevel() == _levelNum)
            {
                ...
                TableCell tableCell = new TableCell((int)getParagraph(lastCellStart).getStartOffset(),
                    (int)getParagraph(p).getEndOffset(), this, _levelNum, tableCellDescriptor, leftEdge,
                    cellWidth);
                cells.add(tableCell);
        ...
        {
            /*logger.log(POILogger.WARN, "Number of found table cells (" + cells.size()
                + ") for table row [" + getStartOffset() + "c; " + getEndOffset()
                + "c] not equals to stored property value " + expectedCellsCount);*/
