...
import office.common.PaintKit;
import office.common.autoshape.ExtendPath;
import office.common.autoshape.pathbuilder.ArrowPathAndTail;
import office.common.bg.BackgroundAndFill;
import office.common.bg.Gradient;
import office.common.bg.LinearGradientShader;
import office.common.bg.RadialGradientShader;
import office.common.bg.TileShader;
import office.common.hyperlink.Hyperlink;
import office.common.pictureefftect.PictureEffectInfoFactory;
import office.common.shape.AbstractShape;
import office.common.shape.ArbitraryPolygonShape;
import office.common.shape.Arrow;
import office.common.shape.AutoShape;
import office.common.shape.GroupShape;
import office.common.shape.IShape;
import office.common.shape.LineShape;
import office.common.shape.PictureShape;
import office.common.shape.ShapeTypes;
import office.common.shape.TableCell;
import office.common.shape.TableShape;
import office.common.shape.TextBox;
import office.constant.AutoShapeConstant;
import office.constant.EventConstant;
import office.constant.MainConstant;
import office.constant.wp.WPAttrConstant;
import office.constant.wp.WPModelConstant;
import office.fc.FCKit;
import office.fc.ShapeKit;
import office.fc.hslf.HSLFSlideShow;
import office.fc.hslf.model.Fill;
import office.fc.hslf.model.HeadersFooters;
import office.fc.hslf.model.Line;
import office.fc.hslf.model.MasterSheet;
import office.fc.hslf.model.Notes;
import office.fc.hslf.model.Shape;
import office.fc.hslf.model.ShapeGroup;
import office.fc.hslf.model.Sheet;
import office.fc.hslf.model.SimpleShape;
import office.fc.hslf.model.Slide;
import office.fc.hslf.model.SlideMaster;
import office.fc.hslf.model.TextShape;
import office.fc.hslf.model.TitleMaster;
import office.fc.hslf.record.BinaryTagDataBlob;
import office.fc.hslf.record.ClientVisualElementContainer;
import office.fc.hslf.record.DocumentAtom;
import office.fc.hslf.record.OEPlaceholderAtom;
import office.fc.hslf.record.PositionDependentRecordContainer;
import office.fc.hslf.record.Record;
import office.fc.hslf.record.SlideAtom;
import office.fc.hslf.record.SlideProgBinaryTagContainer;
import office.fc.hslf.record.SlideProgTagsContainer;
import office.fc.hslf.record.SlideShowSlideInfoAtom;
import office.fc.hslf.record.TimeAnimateBehaviorContainer;
import office.fc.hslf.record.TimeBehaviorContainer;
import office.fc.hslf.record.TimeColorBehaviorContainer;
import office.fc.hslf.record.TimeCommandBehaviorContainer;
import office.fc.hslf.record.TimeEffectBehaviorContainer;
import office.fc.hslf.record.TimeMotionBehaviorContainer;
import office.fc.hslf.record.TimeNodeAttributeContainer;
import office.fc.hslf.record.TimeNodeContainer;
import office.fc.hslf.record.TimeRotationBehaviorContainer;
import office.fc.hslf.record.TimeScaleBehaviorContainer;
import office.fc.hslf.record.TimeSetBehaviorContainer;
import office.fc.hslf.record.TimeVariant;
import office.fc.hslf.record.VisualShapeAtom;
import office.fc.hslf.record.SlideAtom.SSlideLayoutAtom;
import office.fc.hslf.usermodel.PictureData;
import office.fc.hslf.usermodel.RichTextRun;
import office.fc.hslf.usermodel.SlideShow;
import office.fc.ppt.bulletnumber.BulletNumberManage;
import office.fc.ss.usermodel.Textbox;
import office.java.awt.Dimension;
import office.java.awt.Rectangle;
import office.java.awt.Rectanglef;
import office.java.awt.geom.Rectangle2D;
import office.pg.animate.ShapeAnimation;
import office.pg.model.PGModel;
import office.pg.model.PGNotes;
import office.pg.model.PGSlide;
import office.simpletext.font.FontTypefaceManage;
import office.simpletext.model.AttrManage;
import office.simpletext.model.IAttributeSet;
import office.simpletext.model.LeafElement;
import office.simpletext.model.ParagraphElement;
import office.simpletext.model.SectionElement;
import office.ss.util.format.NumericFormatter;
import office.system.AbstractReader;
import office.system.BackReaderThread;
...
public class PPTReader extends AbstractReader
{
    ...
    private office.common.borders.Line getShapeLine(SimpleShape shape)
    {
    	return getShapeLine(shape, false);
    }

    private office.common.borders.Line getShapeLine(SimpleShape shape, boolean isTableCellLine)
    {
        office.common.borders.Line line = null;
        if(shape != null && shape.hasLine())
        {
             ...
             office.java.awt.Color color = ((SimpleShape)shape).getLineColor();

             if(color != null)
             {
            	 line = new office.common.borders.Line();
    ...
    private BackgroundAndFill converFill(PGSlide pgSlide, Fill fill)
    {
        ...
        if (fill != null)
        {
                    ...
                    || type == BackgroundAndFill.FILL_SHADE_SHAPE)
            {
            	...
            	office.java.awt.Color fillColor = fill.getForegroundColor();
            	office.java.awt.Color fillbackColor = fill.getFillbackColor();
    ...
    private void processNotes(PGSlide pgSlide, Notes notes)
    {
        if (notes != null)
        {
            ...
            for (Shape shape : notes.getShapes())
            {
                ...
                if(shape instanceof office.fc.hslf.model.AutoShape  // 文本框
                    || shape instanceof office.fc.hslf.model.TextBox)  // 占位符
                {
...
//        }
        ...
        if (shape instanceof Line
            || shape instanceof office.fc.hslf.model.Freeform
            || shape instanceof office.fc.hslf.model.AutoShape
            || shape instanceof office.fc.hslf.model.TextBox
            || shape instanceof office.fc.hslf.model.Picture)
        {
            if (shape instanceof Line)
            {
                if (line != null)
            ...
            else if(shape instanceof office.fc.hslf.model.Freeform)
            {
                if (fill != null || line != null)
                {
                    ...
                    if (startArrowType > 0)
                    {
                    	ArrowPathAndTail arrowPathAndTail = ((office.fc.hslf.model.Freeform)shape).getStartArrowPathAndTail(rect);
                    ...
                    if (endArrowType > 0)
                    {
                    	ArrowPathAndTail arrowPathAndTail = ((office.fc.hslf.model.Freeform)shape).getEndArrowPathAndTail(rect);
                    	if(arrowPathAndTail != null && arrowPathAndTail.getArrowPath() != null)
                        {
                    ...
                    Path[] paths = ((office.fc.hslf.model.Freeform)shape).getFreeformPath(rect,
            ...
            else if(shape instanceof office.fc.hslf.model.AutoShape
                    || shape instanceof office.fc.hslf.model.TextBox)
            {
                // autoShape
            ...
            // 图片
            else if(shape instanceof office.fc.hslf.model.Picture)
            {
                office.fc.hslf.model.Picture poiPic = (office.fc.hslf.model.Picture)shape;
        ...
        else if(shape instanceof office.fc.hslf.model.Table)
        {
            office.fc.hslf.model.Table poiTable = (office.fc.hslf.model.Table)shape;
            if (poiTable != null)
            {
     ...
     * @param pgSlide
     */
    private void processTable(PGSlide pgSlide, office.fc.hslf.model.Table poiTable, GroupShape parent, int slideType)
    {
        ...
        for (int i = 0; i < rows; i++)
        {
            for (int j = 0; j < columns; j++)
            {
                ...
                office.fc.hslf.model.TableCell poiCell = poiTable.getCell(i, j);
                if (poiCell != null)
                {
        ...
        {
        	office.common.borders.Line aLine = getShapeLine(line, true);
            if (aLine != null)
            {
    ...
    private int getBorderColor(Line line)
    {
        if (line != null)
        {
            office.java.awt.Color color = line.getLineColor();
            if (color != null)
            {
    ...
    private void processNormalTextShape(TextBox tb, TextShape ts, Rectangle rect, int slideType, int placeHolderID)
    {
        ...
        if (text != null && text.trim().length() > 0)
        {
            ...
            int len = text.length();
            office.fc.hslf.model.Hyperlink[] links = ts.getTextRun().getHyperlinks();
            int start = 0;
            // title type just a paragraph needs be processed specially
            if (ts.getTextRun().getRunType() != office.fc.hslf.record.TextHeaderAtom.TITLE_TYPE)
            {
                for (int i = 0; i < len; i++)
                {
                    if (abortReader)
    ...
    private void processParagraph(SectionElement secElem, TextShape ts, String text,
        office.fc.hslf.model.Hyperlink[] links, int start, int end, int placeHolderID)
    {
        ...
        office.fc.hslf.record.TextRulerAtom ruler = ts.getTextRun().getTextRuler();
        if (ruler != null)
        {
...
//      }
     ...
     *
     */
    private int converterColor(office.java.awt.Color color)
    {
//        if(color.getAlpha() == 0)
//        {
